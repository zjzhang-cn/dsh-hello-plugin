 ...
 *
 * harness API 均为进程内访问，本模块类型以结构宽松接口 + cast 表达（宿主 bundle dts: false）。
 */
...
/** ctx.agents（core/agent）的结构化接口。 */
interface AgentsServiceLike {
  create(options: object): Promise<{ agent: AgentLike }>
}
...
interface AgentLike {
  ...
  /** 等待整个 agent 达到静止（模型失败也正常 resolve，不 reject）。 */
 ...
 */
function collectAnalysisText(events: readonly SessionEventLike[]): string {
  let finalText = ''
  ...
  for (const event of events) {
    ...
    if (event.type === 'turn/end') {
      ...
      if (reason?.kind === 'error') {
        failureMessage = typeof reason.error?.message === 'string'
...
export async function runJiraAnalysisSession(
  ...
  ;(ctx as unknown as { sessionTitle: { rename(session: object, title: string): unknown } })
    .sessionTitle.rename(agent.session, `分析 ${key} ${timeStamp()}`)
  ...
  const analysis = collectAnalysisText(agent.session.events.slice(boundary))
